...
export const BreadcrumbList = React.forwardRef<HTMLOListElement, React.ComponentPropsWithoutRef<'ol'>>(
  ({ className, ...props }, ref) => (
    <ol
      ref={ref}
      className={cn(
        'flex flex-wrap items-center gap-1.5 break-words text-[14px] font-bitcell uppercase tracking-[2px] text-giga-muted sm:gap-2.5',
        className,
      )}
      {...props}
    />
...
>(({ asChild, className, ...props }, ref) => {
  ...
  return (
    <Comp
      ref={ref}
      className={cn('transition-colors hover:text-giga-cyan', className)}
      {...props}
    />
  )
})
...
export const BreadcrumbPage = React.forwardRef<HTMLSpanElement, React.ComponentPropsWithoutRef<'span'>>(
  ({ className, ...props }, ref) => (
    <span
      ...
      aria-disabled="true"
      aria-current="page"
      className={cn('text-giga-gold', className)}
      {...props}
    />
  ),
)
